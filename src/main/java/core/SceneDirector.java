...
public class SceneDirector {
  ...
  private void gameRound() {
    ...
    if (frames % NUMBER_OF_FRAMES_OF_ACTIVE_WAVE * FRAME_RATE_SCALING == 0) {
      eventBus.post(
          new GameEvent(
              GameEventType.ATTACK_WAVE_IS_OVER, new GameEventMetaData<>("level", difficulty)));
    }

    if (frames < NUMBER_OF_FRAMES_OF_ACTIVE_WAVE * FRAME_RATE_SCALING
        && 100 * Math.random() * FRAME_RATE_SCALING
            < (difficulty + BASE_CHANCE_FOR_ENEMY_THIS_FRAME)) {
